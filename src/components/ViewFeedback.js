  ...
  return (
    ...
    <div className={styles.tickets}>
      ...
      {tickets.length > 0 ? (
        <ul>
          {tickets.map(ticket => (
  <li onClick={() => setSelectedTicket({id: ticket.id, title: ticket.title})} key={ticket.id}>
    <h3>{ticket.title}</h3>
    ...
    {selectedTicket && <TicketFeedback authToken={authToken} ticketId={selectedTicket.id} ticketTitle={selectedTicket.title} onClose={() => setSelectedTicket(null)} />}
